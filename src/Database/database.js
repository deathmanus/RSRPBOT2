...
function getShopLogs(minutes = 60, callback) {
    ...
    db.all(
        ...
        (err, rows) => {
            ...
            // Převést data JSON zpět na objekty
...
// Inicializace konfigurace ticketů z JSON souboru, pokud není v databázi
function initTicketConfig() {
    return new Promise((resolve, reject) => {
        getTicketConfig((err, config) => {
            ...
            // Pokud konfigurace existuje, není potřeba inicializovat
            if (config) {
                resolve(config);
                return;
            }
            ...
            try {
                ...
                if (fs.existsSync(configPath)) {
                    ...
                    // Uložení konfigurace do databáze
                    ...
                    // Vytvoření prázdné konfigurace, pokud soubor neexistuje
                    const emptyConfig = {
                        categories: [],
                        buttons: {
                            close: {
